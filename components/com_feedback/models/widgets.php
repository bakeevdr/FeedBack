...
class FeedbackModelWidgets extends JModelList
{
	...
	public function __construct($config = array())
	{
		...
		if ($user->guest) { // если зашел гость 
			if ($this -> fb_ui === null) {
				$db->setQuery("SELECT `id`, max(`date_ip`) max_date_ip FROM `#__feedback_user` WHERE ip = '".$_SERVER['REMOTE_ADDR']."'  group by `id` ORDER BY max_date_ip DESC"); // ДОБАВИТЬ УСЛОВИЯ по необходимости
				$db->query(); 
				if ($db->loadRow()[0]===null ) { // добавить запись
					$db->setQuery("Insert into `#__feedback_user` (ip) VALUES ('".$_SERVER['REMOTE_ADDR']."')");
					$db->execute();
				...
				$db->setQuery("Select id, ip from `#__feedback_user` where id='".$this -> fb_ui."'");
				$db->query();
				if ($db->getNumRows() === 0 ) {
					$db->setQuery("Insert into `#__feedback_user` (ip) VALUES ('".$_SERVER['REMOTE_ADDR']."')");
					$db->execute();
					...
					$db->setQuery("Update `#__feedback_user` set ip ='".$_SERVER['REMOTE_ADDR']."', date_ip = now() WHERE id = ".$this -> fb_ui);
					$db->execute();
				}
			}
		}else {  // если зашел авторизованный
			$db->setQuery("Select id, ip from `#__feedback_user` where sys_id='".$user->id."'");
			$db->query(); 
			if ($db->getNumRows () === 0 ) { // добавить запись
				if ($this -> fb_ui === null) {
					$db->setQuery("Insert into `#__feedback_user` (name,	sys_id, ip) VALUES ('".$user->name."',	'".$user->id."','".$_SERVER['REMOTE_ADDR']."')");
					$db->execute();
					$this -> fb_ui = $db->insertid();
				}
				Else {
					...
					if ($db->getNumRows() === 0 ) {
						...
						$db->setQuery("Update `#__feedback_user` set ip ='".$_SERVER['REMOTE_ADDR']."', date_ip = now(), sys_id = '".$user->id."' WHERE id = ".$this -> fb_ui);
						$db->execute();
					}
				}
			} Else {
	...
	function getNicname (){
		$db				= JFactory::getDBO();
		$db->setQuery("Select name from #__feedback_user where id='".$this -> fb_ui."'");
	...
	protected	function getListQuery()
	{
		...
		$query	->select('fbv.status void_user') 
				->leftJoin('`#__feedback_vote` as fbv on fbv.feedback_id = fb.id and fbv.user_id='.$this->fb_ui);/**/
